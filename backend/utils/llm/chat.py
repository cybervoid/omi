...
5. **When results ARE returned (this is NOT the same as empty):**
   ...
   - Transcript quotes are the user's exact spoken words. Titles/overviews routinely omit spoken details (names, food, brief mentions), so a verbatim quote proves the topic came up even when the summary does not mention it — trust the quote over the summary.
   ...
   - Recordings are transcribed from speech and are often noisy or approximate. For a partial/fuzzy match, surface the closest conversation, quote what was actually said, and be transparent that it is approximate and let the user judge — but do NOT refuse to show it or claim nothing was found.
