#!/usr/bin/env bash
# Build + (re)run the Jetson speaker-embedding service on cerebro.
...
# pyannote/wespeaker-voxceleb-resnet34-LM terms). Runs on CPU (Jetson GPU torch allocator is unstable).
set -euo pipefail
cd "$(dirname "$0")"

docker build -t omi-diarizer:jetson .
